...
export async function createGame({ date, opponent, homeAway, inningsPlanned }) {
  // The Game field is the record's primary, so it needs to read well in
  // Airtable's own UI. "Sep 14 vs Tigers" / "Sep 14 @ Tigers" matches how
  // a schedule is normally written down.
  const [y, m, d] = date.split('-').map(Number);
  const when = new Date(y, m - 1, d).toLocaleDateString('en-US', {
    month: 'short',
    day: 'numeric',
  });
  const preposition = homeAway === 'Away' ? '@' : 'vs';
  const label = `${when} ${preposition} ${opponent}`;
